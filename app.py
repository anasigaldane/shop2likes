...
def load_tokens(server):
    files = FILES
    accounts = json.load(open(f"tokens/{files.get(server,'token_bd.json')}"))
    tokens = []
    for acc in accounts:
        uid = acc.get("uid")
        ...
        try:
            r = requests.get(
                f"https://shop2likejwt.vercel.app/token?uid={uid}&password={password}",
                timeout=10
            )
            data = r.json()
            if "token" in data:
                # ناخد JWT بلا كلمة Bearer
                jwt = data["token"].replace("Bearer ", "")
