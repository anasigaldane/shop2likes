...
def like_player():
    ...
    if not tokens_available:
        return jsonify({
            ...
            "message": "All server tokens are missing or expired. Please refresh credentials.",
            "status": 503,
            "timestamp": datetime.utcnow().isoformat()
        }), 503
    ...
    if not player_info:
        return jsonify({
            ...
            "message": f"Player UID {uid} not found on any server",
            "status": 404,
            "timestamp": datetime.utcnow().isoformat()
        }), 404
    ...
    new_info = make_request(encode_uid(uid), f"{SERVERS[region]}/GetPlayerPersonalShow", token_cache.get_tokens(region)[0])
